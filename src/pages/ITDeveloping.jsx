  ...
  return (
    <div className="min-h-screen bg-slate-950 pt-20 pb-16 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="bg-gradient-to-br from-emerald-600/20 to-teal-600/10 border border-emerald-500/30 rounded-2xl p-8 mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="text-4xl mb-3">💻</div>
              <h1 className="text-3xl font-display font-bold text-white mb-2">IT Developing</h1>
              <p className="text-slate-300">Web development, programming fundamentals & software engineering.</p>
            </div>
            <div className="bg-slate-900/60 rounded-xl p-5 min-w-48">
              <p className="text-slate-400 text-sm mb-2">Your Progress</p>
              <p className="text-3xl font-bold text-emerald-400 mb-2">{pct}%</p>
              <div className="w-full h-2 bg-slate-700 rounded-full">
                <div className="h-full bg-gradient-to-r from-emerald-600 to-teal-400 rounded-full transition-all" style={{ width: `${pct}%` }} />
              </div>
        ...
        {progress.lastWatched && (
          <div className="bg-slate-800/60 border border-emerald-500/30 rounded-2xl p-5 mb-8 flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-emerald-600 flex items-center justify-center flex-shrink-0"><PlayCircle size={22} className="text-white" /></div>
            ...
            <Link to={`/lesson/${courseId}/${progress.lastWatched}`} className="flex-shrink-0 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-sm font-medium transition-colors">Resume</Link>
          </div>
        )}
          ...
          {courseLessons.map((lesson) => {
            ...
            return (
              <Link key={lesson.id} to={currentUser ? `/lesson/${courseId}/${lesson.id}` : "/login"}
                className="group flex gap-4 bg-slate-800/60 hover:bg-slate-800 border border-slate-700 hover:border-emerald-500/50 rounded-2xl p-4 transition-all">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="text-white font-medium text-sm mb-1 group-hover:text-emerald-300 transition-colors line-clamp-2">{lesson.title}</h3>
